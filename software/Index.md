...
It is available as [public gist](https://gist.github.com/silvercircle/c86efd6ce26025faa0b95cb5ea316240). It is fairly well commented and depends only on very few plugins.
...
* Copy the ```userChrome.css``` that you'll find inside the .ZIP you downloaded from GitHub into that folder. Under Linux, the case matters, the ```C``` must be capital, the extension lower case. The folder ```chrome``` must be all lowercase.
...
This theme is inspired by the very popular [Dracula theme](https://draculatheme.com/) that is available for many text editors and IDEs. While they do provide a full syntax highlighting color scheme for ST3, there is no "UI Theme", so I decided to make one. I based my work on the popular Soda theme (from the Sublimetext 2 era) and modified it accordingly.
